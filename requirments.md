...
Create readme.md, and claude.md, with detailed plans and explenations. The code should be documented and explained in markdown files.
Each code file, shouldn't exceed 250 code lines. Create a SPA front, with an hamburger menu, including about page, and help page, and settings page.
The front should include log levels, with options to delete logs, and send logs, including logs deletion.
each page should use local storage, to keep the data on it on page refresh, and should keep a collection of the pages that been explored, and links to the pages, in a local storagedb, and a csv file, that will serve as a database, for the information and the search, that the user made.
The project will be deployed to vercel. Create an .env file with an option in the front to add api keys from varius providers, and currently include the below api key from nebius, but add editable option to use openai, gemini, perplexity and claude. Each user can use its own preference and preferd api provider.
SPA Front should be using react and using react MUI framework. The front should be fully responcive to large medium and mobile devices screens.
api key - 
v1.CmQKHHN0YXRpY2tleS1lMDBnYWcxenIzNmtlZTRqd2cSIXNlcnZpY2VhY2NvdW50LWUwMGhxenhuYzY2MG10YXJzejIMCOjki80GELPD088BOgwI5-ejmAcQwMWB6AJAAloDZTAw.AAAAAAAAAAFuzmXA6SJ-Gp7bBCcKjfqP-jjCuKnhHZ3-9y4hugluHGDwax93fMZaED0T3\_arcJhgxMdfyjtIKO-QCk2\_AScC



task link - https://taskchecker.academy.nebius.com/tasks/ai-performance-engineering-2026
Nebius Token Factory - https://tokenfactory.nebius.com/
Nebius API keys - https://tokenfactory.nebius.com/project/api-keys

GitHub repository for commits and push - https://github.com/ikrigel/nebius-admission-assignment
...
github\_url	string	yes	URL of a public GitHub repository
...
You need to decide which files are important and which can be ignored (e.g., binary files, lock files, node\_modules/, etc.)
...
LLM API key is configured via the NEBIUS\_API\_KEY environment variable (we will provide our own key when testing). Do not hardcode API keys. In case you are using an alternative LLM provider, you can use corresponding environment variable name, for example OPENAI\_API\_KEY, ANTHROPIC\_API\_KEY etc.
